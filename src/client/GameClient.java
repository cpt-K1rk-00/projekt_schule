...
/**
 * Die Klasse dient der Kommunikation zwischen dem Server und der GUI. In der
 * Process Message Methode werden die Daten empfangen, die dann gegebenfalls
 * lokal gespeichert werden mï¿½ssen.
 */
public class GameClient extends Client {
	...
	public void processMessage(String pMessage) {
		...
		if (msg[0].equals("1")) {
			...
			if (msg[1].equals("Anmeldung erfolgreich")) {
				...
				Platform.runLater(new Runnable() {
					public void run() {
						gui.showError("Anmeldedaten falsch");
					}
				});
				...
				Platform.runLater(new Runnable() {
					public void run() {
						gui.showError("Fehler bei Anmeldung");
					}
				});
			}
			...
			if (msg[1].equals("Registrierung erfolgreich")) {
				...
				Platform.runLater(new Runnable() {
					public void run() {
						gui.showError("Nutzername bereits vergeben");
					}
				});
				// Wenn es einen Fehler gab
				...
				Platform.runLater(new Runnable() {
					public void run() {
						gui.showError("Fehler bei Registrierung");
					}
				});
			}
			// Freunde laden
			...
			if (msg[1].equals("Laden erfolgreich")) {
				...
				Platform.runLater(new Runnable() {
					public void run() {
						gui.showError("Freunde konnten nicht geladen werden");
					}
				});
			}
				...
				if (msg[2].equals("keine Liga")) {
					...
					Platform.runLater(new Runnable() {
						public void run() {
							...
							gui.getRoot().setCenter(leagueView);
						}
					});
				}
			...
			if (msg[1].equals("Fehler beim Verlassen der Liga")) {
				Platform.runLater(new Runnable() {
					public void run() {
						gui.showError("Fehler beim Verlassen der Liga");
					}
				});
				// Liga neu Laden
			} else if (msg[1].equals("Lade Liga")) {
			...
			if (msg[1].equals("Liga beigetreten")) {
				...
				Platform.runLater(new Runnable() {
					public void run() {
						gui.showError("Fehler beim Beitritt der Liga");
					}
				});
			}
	...
	/**
	 * Stellt eine Anfrage auf eine Liste mit allen Mitgliedern einer Liga.
	 * 
	 * @param pLeagueName
	 */
